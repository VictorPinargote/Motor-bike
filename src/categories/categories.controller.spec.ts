...
describe('CategoriesController', () => {
  ...
  describe('create', () => {
    it('debe crear una categoría y retornar la respuesta de éxito', async () => {
      const dto = { nombre: 'Deportiva', descripcion: 'Motos de alto rendimiento' };
      const categoria = { id: '1', ...dto };
      service.create.mockResolvedValue(categoria as any);

      const result = await controller.create(dto as any);

      expect(result).toEqual({ success: true, message: 'Categoría creada correctamente', data: categoria });
    });
  ...
  describe('findOne', () => {
    it('debe retornar una categoría existente', async () => {
      const categoria = { id: '1' };
      service.findOne.mockResolvedValue(categoria as any);

      const result = await controller.findOne('1');

      expect(result).toEqual({ success: true, message: 'Categoría encontrada', data: categoria });
    });
  ...
  describe('update', () => {
    it('debe actualizar una categoría existente', async () => {
      ...
      const categoria = { id: '1', nombre: 'Urbana' };
      service.update.mockResolvedValue(categoria as any);

      const result = await controller.update('1', dto as any);

      expect(result).toEqual({ success: true, message: 'Categoría actualizada', data: categoria });
    });
  ...
  describe('remove', () => {
    it('debe eliminar una categoría existente', async () => {
      const categoria = { id: '1' };
      service.remove.mockResolvedValue(categoria as any);

      const result = await controller.remove('1');

      expect(result).toEqual({ success: true, message: 'Categoría eliminada', data: categoria });
    });
